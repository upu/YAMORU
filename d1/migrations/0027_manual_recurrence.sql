-- Issue #325 / YDR-046: 「必要になったら繰り返す」Todoを
-- recurrence_basis = 'manual' として追加する。manualのOccurrenceは常に
-- scheduled_for / due_at がNULLで、完了すると次の予定日未定Occurrenceが1件
-- 作られる。
--
-- recurrence_basisの値とそれに伴う列の組み合わせはtask_rulesの複合CHECK制約で
-- 表現されており、SQLiteではCHECK制約の列挙をALTER TABLEで拡張できない
-- (0016・0023と同じ理由)。そのため0023と同じ手順でtask_rulesを再作成する。
-- task_rulesを削除するとON DELETE CASCADEが子へ波及するため(D1は
-- foreign_keys = 1)、子孫テーブル(task_occurrences、activity_logs、
-- completion_corrections、task_rule_consumables、task_rule_changes、
-- task_rule_schedules)も0023と同じ順序で作り直し、既存データ・外部キー・
-- 制約・索引・トリガーを保持する。列の定義は0023時点から変えない。
--
-- 0009が「予定日未定は一回限りTodoだけ」とした制約は、'once'に加えて
-- 'manual'を許す形へ広げる(YDR-046がYDR-030のこの部分だけを置き換える)。
-- 逆向きの不変条件(manualのOccurrenceに日付を入れない)は、このmigrationで
-- 追加するトリガーがINSERT・UPDATEの両方で守る。
--
-- ロールアウト: 追加は許可の拡張だけであり、migration適用直後の旧Workerは
-- これまでと同じ書き込み・読み取りを続けられる。旧Workerはmanualの行を
-- 作らないため、互換期間中にmanualの行が現れるのは新Workerが出た後だけに
-- なる。その後に旧Workerへ戻した場合、旧Workerはmanualを未知の
-- recurrence_basisとして扱い、Todo一覧・完了で明示的に失敗する(0016で
-- 'interval'を追加したときと同じ扱い。誤った日付を黙って作ることはない)。
...
-- 0023までのトリガーを、再作成したテーブルへ作り直す。予定日未定に関する
-- 三つ(0009由来)は'manual'を許す条件へ広げ、名前も対象の方式が分かる形へ
-- 変える。他のトリガーは0023と同じ定義のまま。
...
-- YDR-046: manualは「日付を決めない」方式そのものなので、一回限りTodoと違って
-- 具体日と未定を往復しない。三つのトリガーで、manualのOccurrenceに日付が
-- 入らないことをINSERT・UPDATE・方式変更の各経路から守る。
