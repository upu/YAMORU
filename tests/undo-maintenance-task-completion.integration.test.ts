...
describe("メンテナンスTodo完了取消の実Auth・実DB接続", () => {
  it("完了を取り消すと、詳細画面が使うクエリで元の未完了Todoと完了取消の履歴を確認できる", async () => {
    ...
    const undoResult = await supabase.rpc("undo_maintenance_task_completion", {
      idempotency_key: crypto.randomUUID(),
      occurrence_id: occurrenceId,
    });
    expect(undoResult.error).toBeNull();
    expect(undoResult.data).toBe(occurrenceId);
    ...
    // 未変更の次回Occurrenceは削除され、元の1件だけが残る。
    expect(occurrences).toHaveLength(1);
    const [reverted] = occurrences;
    expect(reverted.status).toBe("pending");
    expect(reverted.scheduled_for).toBe("2020-01-01T00:00:00+00:00");
    expect(reverted.due_at).toBe("2020-01-08T00:00:00+00:00");

    const actions = reverted.activity_logs
      .map((log) => log.action)
      .sort((left, right) => left.localeCompare(right));
    // 完了ActivityLogは削除・上書きされず、取消ActivityLogが追記される。
    expect(actions).toEqual(["completed", "completion_undone"]);
