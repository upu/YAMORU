...
Cloudflareへの本番デプロイ、リモートD1の対象選択と誤操作ガード、既存実データ移行はIssue #123以降で扱う。現在の`auth:bootstrap`と`auth:reset-password`はローカルD1だけを対象とする。
...
2. `auth:bootstrap`で登録したメールアドレスと12文字以上のパスワードでログインする。
