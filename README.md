...
すでにホーム画面へ追加済みでアイコンが更新された場合は、既存のYAMORUアイコンを削除してからSafariで追加し直します。それでも追加画面に古い画像や「Y」が表示される場合は、iPhoneの「設定 > アプリ > Safari > 詳細 > Webサイトデータ」で開発用ホストのデータを削除し、Safariで開き直してから追加します。この操作では対象ホストからログアウトするため、再ログインが必要です。
