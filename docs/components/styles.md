...
| 台帳のページ見出し | `src/app/ledger-page.module.css` |
...
- サイドバーは共通ヘッダーの次(本文より前)に置く([Issue #219](https://github.com/upu/YAMORU/issues/219))。画面ではヘッダーの下に立つため、キーボードの移動順も「ヘッダー → サイドバー → 本文」と画面の並びにそろえる。
- 主要ナビゲーションは画面幅で見た目だけを変え、項目・行き先・現在地の意味は`src/app/primary-navigation.tsx`の一つの値から両方へ渡す([Issue #219](https://github.com/upu/YAMORU/issues/219))。モバイル幅(564px以下)は下部ナビゲーション、それより広い幅はサイドバー。切り替えは`display: none`で行い、隠れている側は支援技術からも見えないようにする(追加導線 #391 と同じ方法)。境界を下部ナビゲーションが消える幅とそろえ、主要ナビゲーションが無い幅を作らない。その境界(565px)は「サイドバーを出したあとの本文の幅」から決める。サイドバーは本文の幅を削るため、本文がこれまで下部ナビゲーションなしで扱っていた幅(481px以上)を下回らないよう、折りたたんだサイドバーの幅84pxを足した565pxを下限にする(565 - 84 = 481、900 - 216 = 684)。これで、画面の幅で切り替わる本文側の指定(フォームの列組みなど)が想定より狭い幅で効くことがない。モバイル幅の指定(下部ナビゲーション、右下の追加ボタン、ダイアログの下端、ヘッダー)も同じ564pxで切り替え、境界を一つに保つ。
- サイドバーは565〜899pxでアイコンの下に語を置いて折りたたみ、900px以上で語を横へ並べて展開する([Issue #219](https://github.com/upu/YAMORU/issues/219))。折りたたんでも語は消さない(記号だけにすると行き先が読み取れない)。利用者が開閉する状態は持たないため、保存する開閉状態もない。
- サイドバーの幅は`--sidebar-width`(`globals.css`の`:root`、モバイル幅では`0px`)に持つ([Issue #219](https://github.com/upu/YAMORU/issues/219))。サイドバー自身の幅、`app-shell.module.css`の左padding(ヘッダー・本文・フッターを右へ寄せる)、画面上部へ固定する通知の幅(`calc(100vw - var(--sidebar-width) - 余白)`)と、画面いっぱいを基準に置く通知(Todo登録の完了通知)の中心(`left: calc(50% + var(--sidebar-width) / 2)`)が同じ値を参照する。共通ヘッダーの中にある更新結果の通知は`left: 50%`のままにする。ヘッダーが`backdrop-filter`を持ち`fixed`の基準になり、そのヘッダー自身がすでにサイドバーの右側へ寄っているため、さらにずらすと二重にずれる。画面いっぱいを基準にすると、中間幅で通知の左端がサイドバーに隠れ、右端が画面の外へ出る。右下の追加ボタンは画面の反対側に固定するため、サイドバーとは重ならない。
...
- 本文の最大幅は画面種別が決め、全画面で一つの幅を使わない([Issue #397](https://github.com/upu/YAMORU/issues/397))。`main`は`--page-max-width`を読み、`.page-form` / `.page-list` / `.page-detail`のどれかがその値を`globals.css`の`:root`の`--page-width-form`(760px) / `--page-width-list`(1160px) / `--page-width-detail`(1000px)から与える。種別を付け忘れた画面はいちばん狭いフォームの幅へ落ちる(広げるのは意図したときだけにする)。ログイン画面(`.auth-page`)だけは2枚のカードを横に並べるため、3種別に入れず自前の900pxを持つ。フォームを広げないのは、入力欄が幅いっぱいに伸びるため、最大幅を広げると入力欄と補足文の行長だけが伸びて読みにくくなるから。フォームがPCの幅を使うのは「横に並べられる短い入力(年・月・日など)」であって、最大幅ではない。ホームは追加導線の規約(#391)では一覧ではないが、並んでいるのは一覧と同じカードなので、幅と段組みは`.page-list`として扱う。
- 一覧画面で2列にするのは一覧本体(`.card-list`、`.ledger-list`)だけで、ページ全体を段組みにしない([Issue #397](https://github.com/upu/YAMORU/issues/397))。詳細画面のカードの中にも同じ`.ledger-list`があるため、`.page-list`の中のものだけを対象にする。境界(972px)は「2列にしたあとの1列の幅」から決める。一覧はカードの中にあるので、モバイル(390px)での1行の幅326pxを下回らないよう、326 × 2 + 10(gap) + 44(カードの左右padding) + 2(border) + 48(mainの左右padding) + 216(サイドバー)を境界にする。一覧を広げても、検索・絞り込みの1行の入力(`.ledger-search-form`)はフォーム画面の入力欄と同じ666pxまでしか伸ばさない。1行の入力は幅を増やしても読みやすさが上がらず、右端の送信ボタンだけが遠くなる(Todo一覧の画面内検索が#390で同じ理由の上限を持つ)。
- 詳細画面の2カラムは、カードを並べ替えずに主要情報(`.detail-column-main`)と関連情報(`.detail-column-side`)へ包んで作る([Issue #397](https://github.com/upu/YAMORU/issues/397))。2カラムにしない幅では包みを`display: contents`で消し、これまでどおりDOM順のまま縦に積む。これで#395で決めた重要度順の並びが画面幅で変わらない。境界(1083px)は、主カラムが本文の下限とした481px(#219)、副カラムが対応する最小の画面幅320pxを保つ幅から決め、列の比(3fr : 2fr)はその境界でちょうど481px : 320pxになる。関連情報を一枚も持たない対象(関連する消耗品も実施記録も無いTodoなど)では、`:has(> .detail-column-side:empty)`で空の列を作らず1カラムへ戻す。
- ページ見出しの大きさは、その画面が読み物・詳細か、一覧・入力かで決める([Issue #400](https://github.com/upu/YAMORU/issues/400))。詳細画面は`.detail-hero`と大きな`h1`(`globals.css`の`clamp()`)のまま。一覧・登録・編集の画面は1.3remのページ見出しに収め、本文を早く見せる(Todo一覧の`.toolbarTitle`、登録・編集の`.form-page-title`、台帳の`ledger-page.module.css`の`.title`が同じ大きさ)。台帳を小さくするために全画面の`h1`を一括で変えない。英字のキッカー(`HOUSE LEDGER`など)は、すぐ下の見出しと同じことしか示さないときは置かない(#237・#327・#393と同じ考え方)。
- ヘルプ(`src/app/help/page.tsx`)の「YAMORUの使い方」は、画面の作りと同じ概念の重なりで説明する([Issue #399](https://github.com/upu/YAMORU/issues/399))。大きな役割は「やること(Todo)」と「家で管理しておくもの(台帳)」の2つとし、備品・サービス・契約・消耗品は台帳の説明の中に入れる。消耗品はDBではManagedItemと別のテーブルだが、画面では台帳の3つ目の入口(`ledger-category-navigation.tsx`の「台帳の種類」)なので、Todo・台帳・消耗品を同列の3概念として並べない。種類の名前は台帳画面と同じ語を使い、ヘルプだけの用語を作らない。各種類の説明は、その大分類で登録できる対象を狭く見せない([YDR-036](../decisions/ydr-036-merge-service-obligation-kinds.md))。「サービス・契約」はサービス提供・契約に加えて税金や公的支払いも含む(詳しい種類のプリセットは契約・習い事・固定資産税)ため、契約だけを挙げない。
...
- 一覧(Todo・備品・サービス・契約・消耗品)の追加導線は、同じ画面へ2つ出さない([Issue #391](https://github.com/upu/YAMORU/issues/391))。モバイル幅(564px以下)は右下のフローティングボタン、それより広い幅は一覧の操作行の先頭のリンク(`.list-add-link`)だけを出す。切り替えは`display: none`で行い、隠れている側は支援技術からも見えないようにする。文言と行き先は`ListAddAction`の同じ値を両方へ渡し、画面幅で名前が変わらないようにする。ホームは一覧ではないため、右下のボタンをどの幅でも出す。
...
主要ナビゲーションが画面幅で入れ替わること、サイドバーが本文を覆わないこと、キーボードと文字サイズ2倍で使えること、公開画面に出ないことは`e2e/sidebar-navigation.spec.ts`が390px・564/565px(境界)・600px・768px・1280pxで確認する。

押せる領域は`e2e/tap-target-size.spec.ts`が390pxと320pxで確認する。要素の大きさだけでなく、中心から21px離れた点を押したときにその操作へ当たるかどうかで見るため、疑似要素で広げた領域も対象になる。

台帳のページ見出しが詳細画面の大見出しより小さいこと、ヘッダーが本文を押し下げないこと、カテゴリ切り替え・検索・追加がそのまま使えることは`e2e/ledger-header-compact.spec.ts`が390pxと320pxで確認する。

画面種別ごとの最大幅、一覧と詳細がPC幅で2列・2カラムになること、モバイル幅では単一カラムのまま横スクロールが出ないこと、段組みが切り替わる境界(972px・1083px)とそのときの各列の幅は`e2e/wide-screen-layout.spec.ts`が1440pxと390px、および境界の前後で確認する。
