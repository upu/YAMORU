...
- データベースのテーブル・カラム・RPC関数を確認するときは、`supabase/migrations/`を通読せず、生成物である`lib/supabase/database.types.ts`を読む。RLSポリシーは型に含まれないため、`supabase/tests/database/`のpgTAPテストを併せて読む。
