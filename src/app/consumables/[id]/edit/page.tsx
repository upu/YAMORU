...
export default async function ConsumableEditPage({
  ...
  const [consumable, options] = await Promise.all([
    getConsumable(db, session, id),
    listConsumableRelationOptions(db, session),
  ]);
  ...
  return (
    <main className="detail-page">
      ...
      <section aria-labelledby="edit-consumable-title" className="detail-card">
        ...
        <ConsumableForm consumable={consumable} mode="edit" options={options} />
      </section>
    </main>
  );
}
