import type { ReactNode } from "react";

// Issue #219: 主要ナビゲーションの行き先と現在地の判定を、モバイルの下部
// ナビゲーション(mobile-bottom-navigation)とPCのサイドバー
// (sidebar-navigation)で共有する。画面幅で見た目は変わっても、項目・行き先・
// 現在地の意味は一つにする(#391で追加導線の文言と行き先を一つの値から渡した
// のと同じ考え方)。モバイルの情報設計そのものは変えない。

function HomeIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24">
      <path d="m3.5 10.5 8.5-7 8.5 7" />
      <path d="M5.5 9v11h13V9M9.5 20v-6h5v6" />
    </svg>
  );
}

function TodoIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24">
      <path d="m3.5 6 1.4 1.4L7.5 4.8M9.5 6H20" />
      <path d="m3.5 12 1.4 1.4 2.6-2.6M9.5 12H20" />
      <path d="m3.5 18 1.4 1.4 2.6-2.6M9.5 18H20" />
    </svg>
  );
}

function LedgerIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24">
      <path d="M5 4.5h11.5A2.5 2.5 0 0 1 19 7v12.5H7A2 2 0 0 1 5 17.5v-13Z" />
      <path d="M5 17.5A2.5 2.5 0 0 1 7.5 15H19M9 8h6" />
    </svg>
  );
}

// Issue #350 / YDR-042: 横断検索は「画面を選ばず名前から対象へ到達する」
// 日常操作なので、補助メニューの奥ではなく主要ナビゲーションへ常設する
// (Issue #242の案1)。共通ヘッダーへ虫眼鏡を置く案は、Todo一覧の画面内検索の
// 虫眼鏡(#241)と意味が重なるため採らない。
function SearchIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24">
      <circle cx="11" cy="11" r="6.5" />
      <path d="m15.8 15.8 4.2 4.2" />
    </svg>
  );
}

export type PrimaryNavigationItem = {
  href: string;
  icon: ReactNode;
  // 現在地の判定に使う、この項目が受け持つパス。prefixはその配下も含める。
  isCurrent: (pathname: string) => boolean;
  key: string;
  label: string;
};

function exactOrUnder(...paths: string[]): (pathname: string) => boolean {
  return (pathname) => paths.some(
    (path) => pathname === path || pathname.startsWith(`${path}/`),
  );
}

// 台帳は備品・サービス/契約(managed-items)と消耗品(consumables)の両方を
// 受け持つ。URLとデータモデルは分けたまま、家庭から見た「家の台帳」という
// 一つの現在地として扱う(#291)。
export const PRIMARY_NAVIGATION_ITEMS: PrimaryNavigationItem[] = [
  {
    href: "/",
    icon: <HomeIcon />,
    isCurrent: (pathname) => pathname === "/",
    key: "home",
    label: "ホーム",
  },
  {
    href: "/todos",
    icon: <TodoIcon />,
    isCurrent: exactOrUnder("/todos"),
    key: "todos",
    label: "Todo",
  },
  {
    href: "/managed-items?kind=asset",
    icon: <LedgerIcon />,
    isCurrent: exactOrUnder("/managed-items", "/consumables"),
    key: "ledger",
    label: "台帳",
  },
  {
    href: "/search",
    icon: <SearchIcon />,
    isCurrent: (pathname) => pathname === "/search",
    key: "search",
    label: "検索",
  },
];
